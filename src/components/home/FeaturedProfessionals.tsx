...
import {type Professionals} from "../../types/database";
import {
  Carousel,
  CarouselContent,
  CarouselItem,
...
// Fallback service teams data (same as original for fallback)
const fallbackServiceTeams = [
  {
    id: "cleaning",
    ...
    verified: true,
    image:
      "https://images.unsplash.com/photo-1578329824171-ef7611e9a5ca?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwcm9mZXNzaW9uYWwlMjBjbGVhbmluZyUyMHRlYW18ZW58MXx8fHwxNzYwMzg4MTEzfDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
  ...
  },
  {
    ...
    verified: true,
    image:
      "https://images.unsplash.com/photo-1568139510872-a900ba9b46da?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwbHVtYmVyJTIwd29ya2VyJTIwdW5pZm9ybXxlbnwxfHx8fDE3NjA0Mjc3MzF8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
  ...
  },
  {
    ...
    verified: true,
    image:
      "https://images.unsplash.com/photo-1660330589693-99889d60181e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxlbGVjdHJpY2lhbiUyMHdvcmtpbmd8ZW58MXx8fHwxNzYwNDI2MTE1fDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
  ...
  },
  {
    ...
    verified: true,
    image:
      "https://images.unsplash.com/photo-1752649935095-ac8f23ec446b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwYWludGVyJTIwcHJvZmVzc2lvbmFsfGVufDF8fHx8MTc2MDM3MDE1OHww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
  ...
  },
  {
    ...
    verified: true,
    image:
      "https://images.unsplash.com/photo-1611134313089-c28c32796751?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxoYW5keW1hbiUyMHRvb2xzJTIwd29ya3xlbnwxfHx8fDE3NjAzNTMzMDB8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
...
const ProfessionalSkeleton = () => (
  <div className='bg-card rounded-xl shadow-sm border border-border h-full flex flex-col animate-pulse'>
...
export default function FeaturedProfessionals() {
  ...
  // Fetch professionals from Appwrite
  useEffect(() => {
    const fetchProfessionals = async () => {
      try {
        ...
        const transformedTeams: ServiceTeam[] = response.documents.map(
          (doc: any) => {
            const professional = doc as Professionals;

            // Create service team data from professional data
            const specialtiesArray = professional.specialization || [
              "General Services",
            ];
            const serviceName = specialtiesArray[0] + " Services";

            return {
              id: professional.$id,
              serviceName: serviceName,
              description: professional.bio || "Professional service provider",
              teamSize: `${
                professional.years_of_experience || 1
              }+ Years Experience`,
              rating: professional.average_rating || 4.5,
              reviews: Math.floor(Math.random() * 500) + 100, // Random reviews for demo
              completedJobs: `${Math.floor(Math.random() * 1000) + 500}+`,
              startingPrice: Math.floor(Math.random() * 300) + 99,
              specialties: specialtiesArray.slice(0, 3),
              location: "UAE",
              verified: true,
              image: `https://images.unsplash.com/photo-${
                Math.floor(Math.random() * 1000000000) + 1500000000
              }-${Math.floor(
                Math.random() * 1000000000
              )}?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=400&h=300`,
              badgeColor: getBadgeColor(specialtiesArray[0]),
            };
          }
        );

        setServiceTeams(
          transformedTeams.length > 0 ? transformedTeams : fallbackServiceTeams
        );
        setError(null);
      } catch (err) {
        ...
        setServiceTeams(fallbackServiceTeams);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Helper function to get badge color based on specialization
  const getBadgeColor = (specialization: string): string => {
    const colorMap: Record<string, string> = {
      Cleaning: "bg-emerald-600",
      Plumbing: "bg-blue-600",
      Electrical: "bg-yellow-600",
      Painting: "bg-purple-600",
      Handyman: "bg-orange-600",
      Maintenance: "bg-teal-600",
      Installation: "bg-indigo-600",
    };
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='text-center mb-12'>
          ...
          <p className='text-lg text-muted-foreground max-w-2xl mx-auto'>
            All our service professionals are directly employed, trained, and
            certified by AL-Kabir for consistent quality and reliability
          </p>
          {error && (
        ...
        <Carousel
          ...
          <CarouselContent className='-ml-2 md:-ml-4'>
            {loading
              ...
              : serviceTeams.map((team: ServiceTeam) => (
                  <CarouselItem
                    key={team.id}
                    className='pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3 xl:basis-1/4'>
                        ...
                        <div className='aspect-[4/3] overflow-hidden relative'>
                          ...
                          <div className='absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent'>
                            <div className='absolute bottom-3 left-3 right-3'>
                              <Badge
                                className={`${team.badgeColor} text-white border-0`}>
                                {team.teamSize}
                              </Badge>
                            </div>
                          </div>
                          ...
                          <div className='flex items-center justify-between pt-3 border-t border-border mt-auto'>
                            <div>
                              ...
                              <div className='text-lg text-emerald-600 dark:text-emerald-400'>
                                AED {team.startingPrice}
                              </div>
                            </div>
                            <Button
                              size='sm'
                              className='bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-700 group-hover:translate-x-1 transition-transform'>
                              Book
        ...
        <div className='bg-gradient-to-br from-emerald-600 to-teal-700 dark:from-emerald-700 dark:to-teal-800 rounded-2xl p-8 text-white mb-8 transition-colors duration-300'>
          <h3 className='text-2xl mb-6 text-center'>
            Why Choose AL-Kabir Service Teams?
          </h3>
          <div className='grid md:grid-cols-3 gap-6'>
            <div className='text-center'>
              ...
              <p className='text-emerald-100 dark:text-emerald-200 text-sm'>
                Background checks, training & certifications for all team
                members
              </p>
            </div>
            <div className='text-center'>
              ...
              <p className='text-emerald-100 dark:text-emerald-200 text-sm'>
                All professionals are AL-Kabir employees, not freelancers
              </p>
            </div>
            <div className='text-center'>
              <div className='bg-white/20 dark:bg-white/10 rounded-full w-12 h-12 flex items-center justify-center mx-auto mb-3'>
              ...
              <p className='text-emerald-100 dark:text-emerald-200 text-sm'>
                Standardized training ensures reliable service every time
              </p>
            </div>
          </div>
        </div>
