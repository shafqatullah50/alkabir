...
export default function AuthTestToggle() {
  const { testMode, toggleTestAuth, user } = useAuth();
  ...
  return (
    ...
    >
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between pb-3 border-b border-border">
                ...
                <div className={`px-2 py-1 rounded-full text-xs ${testMode ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>
              ...
              {testMode && user && (
                <div className="flex items-center space-x-3 p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg border border-emerald-200 dark:border-emerald-800">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-foreground truncate">{user.user_metadata?.full_name}</p>
      ...
      <Button
        ...
        size="lg"
        className={`rounded-full shadow-2xl ${testMode 
          ? 'bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-700' 
