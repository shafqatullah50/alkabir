...
export default function Header() {
  ...
  return (
    <header className='fixed top-0 left-0 right-0 z-50 bg-background border-b border-border shadow-sm transition-colors duration-300'>
      <div className='w-full px-4 md:max-w-7xl md:mx-auto md:px-6 lg:px-8'>
        <div className='flex justify-between items-center h-16 md:h-18'>
          {/* Logo - Mobile First */}
          <Link
            to='/'
            className='flex items-center space-x-1.5 md:space-x-2 group flex-shrink-0'>
                      ...
                      {topServices.map((service) => {
                        ...
                        return (
                          <Link
                            ...
                            onClick={() => setMobileMenuOpen(false)}>
                            <div
                              className={`${service.bgColor} dark:bg-opacity-20 ${service.color} p-2.5 rounded-xl flex-shrink-0`}>
                              <Icon className='w-5 h-5' />
                            </div>
                            <div className='flex-1 min-w-0'>
                              <h4 className='text-sm font-medium text-foreground'>
