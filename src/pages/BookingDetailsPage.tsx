...
import { Calendar, Clock, MapPin, User, CreditCard, Phone, Mail, FileText, CheckCircle, ArrowLeft, Download, Star, MessageSquare } from 'lucide-react';
...
export default function BookingDetailsPage() {
  ...
  const booking = {
    ...
    description: 'Deep cleaning service for 2-bedroom apartment including kitchen, bathrooms, and all living areas.',
    ...
    <div className="min-h-screen bg-background transition-colors duration-300 py-8">
      <div className="w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="bg-card rounded-2xl shadow-sm p-6 md:p-8 mb-6 border border-border transition-colors duration-300">
          ...
          {/* Professional Info */}
          <div>
            <h3 className="text-lg text-foreground mb-4">Assigned Professional</h3>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div>
                      <h4 className="text-lg text-foreground">{booking.professional.name}</h4>
                      <div className="flex items-center gap-4 mt-1">
                        <div className="flex items-center gap-1">
                          <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                          <span className="text-sm text-foreground">{booking.professional.rating}</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <CheckCircle className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                          <span className="text-sm text-muted-foreground">{booking.professional.completedJobs} jobs</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-3 mt-3">
                    <a href={`tel:${booking.professional.phone}`} className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400 hover:underline">
                      <Phone className="w-4 h-4" />
                      {booking.professional.phone}
                    </a>
                    <a href={`mailto:${booking.professional.email}`} className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400 hover:underline">
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/30 rounded-xl p-4 sm:p-6 border border-border">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4">
                  ...
                  <div className="sm:text-right">
                    ...
                    <p className="text-sm sm:text-base text-foreground">{booking.bookingNumber}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-border pt-3 sm:pt-4 space-y-2 sm:space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-foreground text-right">{booking.service}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-foreground text-right">{booking.professional.name}</span>
                  </div>
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">AED {(booking.price * 0.05).toFixed(2)}</span>
                  </div>
                  <div className="border-t border-border pt-2 sm:pt-3 flex justify-between">
                    ...
                    <span className="text-base sm:text-lg text-emerald-600 dark:text-emerald-400">AED {(booking.price * 1.05).toFixed(2)}</span>
                  </div>
                </div>
              </div>
        ...
        <Dialog open={reviewModalOpen} onOpenChange={setReviewModalOpen}>
          <DialogContent className="w-[calc(100%-2rem)] max-w-lg mx-auto max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription className="text-sm">
                Share your experience with {booking.professional.name} to help others make informed decisions.
              </DialogDescription>
            </DialogHeader>
